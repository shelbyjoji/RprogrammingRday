...
library(effects)
M1<-(glm(heart_disease_present ~ exercise_induced_angina*oldpeak_eq_st_depression, family=binomial(link = 'logit'), data=train))
plot(allEffects(M1))
...
```{r}
final<-(glm(heart_disease_present ~ factor(num_major_vessels)+ factor(thalach)+ factor(chest_pain_type)+factor(exercise_induced_angina)*oldpeak_eq_st_depression, family=binomial(link = 'logit'), data=train))
